...
makePCountNetworkWide_umf<-function(DataIn,SpeciesIn){
  require(unmarked)
  require(plyr)
  require(reshape2)
  ...
  #create column for Network
  data.new$Network_Code<-"NCRN"
  
  #create columns
  data.new$Year.Unit.Plot.Visit<-paste(data.new$Year, data.new$Network_Code, data.new$Plot_Name, data.new$ Visit,sep="_")
  ...
  data.new$Year.Unit.Plot<-paste(data.new$Year, data.new$Network_Code, data.new$Plot_Name,sep="_")
  ...
  data.count.melt <- melt(data.new, id=c("Network_Code", "Unit_Name", "Unit_Code","Plot_Name","Year", "Month","Day","Ord.Day","Date","Visit","Lat_WGS84","Long_WGS84", "Start_Time.dec","Interval", "AOU_Code","Sex_ID","Distance_id","Observer","Visit.Interval","Year.Unit.Plot"),
  ...
  head(data.new)
  data.new.covs<-data.new[,c("Year.Unit.Plot","Year","Network_Code","Unit_Code","Plot_Name","Visit", "Long_WGS84","Lat_WGS84","Date","Month","Ord.Day","Start_Time.dec","Temperature","Wind_Speed","Observer")]
  ...
  #format count data
  covs.melt<- melt(data.new.covs.unique,id=c("Year.Unit.Plot","Year","Network_Code","Unit_Code","Plot_Name","Visit"), measure=c("Ord.Day","Month","Start_Time.dec","Temperature","Wind_Speed","Observer"))
  head(covs.melt)
